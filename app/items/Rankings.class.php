...
// start read sql data    
    ...
    /**
     * List of Rankings of all Days in the current month/year
     *
     * @param  array $days_list list of Days ids in the current month/year
     * @return array
     */
    static function rankings_month_list($days_list) : array
    {
        $days_list = implode(',', $days_list);
        $rankings_list = array();
        $req = 'SELECT * FROM ' . Config::get_config()->get('db_prefix') . 'rankings '
        . ' WHERE day_id IN (' . $days_list . ')'
        . ' ORDER BY victory DESC, pos_points DESC, neg_points DESC, member_name ASC';
        if ($result = Connection::query($req)){
            if (!empty($result)){
                foreach ($result as $value)
        ...
        $totals = array();
        foreach ($rankings_list as $row) {
            $memberId = $row['member_id'];
            if (isset($totals[$memberId])) {
                // If member already exists
                $totals[$memberId]['member_id'] = $row['member_id'];
                $totals[$memberId]['member_name'] = $row['member_name'];
                $totals[$memberId]['played'] += $row['played'];
                $totals[$memberId]['victory'] += $row['victory'];
                $totals[$memberId]['loss'] += $row['loss'];
                $totals[$memberId]['pos_points'] += $row['pos_points'];
                $totals[$memberId]['neg_points'] += $row['neg_points'];
            } else {
                // Si le member_id n'existe pas encore, initialise les totaux
                $totals[$memberId] = array(
                    'member_id' => $row['member_id'],
                    'member_name' => $row['member_name'],
                    'played' => $row['played'],
                    'victory' => $row['victory'],
                    'loss' => $row['loss'],
                    'pos_points' => $row['pos_points'],
                    'neg_points' => $row['neg_points'],
                );
            }
        }
        ...
        array_multisort($sort, SORT_DESC, $totals);
        return $totals;
    }
